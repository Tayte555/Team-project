    ...
    <div>
        <div class="md:ml-[8.3333%] md:mr-[8.3333%] px-4 py-8 lg:py-12">
            <div class="gap-4 lg:grid lg:grid-cols-2">
                <h1 class="text-3xl font-bold tracking-wide">Best Sellers 🔥</h1>
                <h2
                    ...
                    Shop the best selling products available at Solehaven, from Air Jordan, Yeezy, Nike and
                    Off-white
                </h2>
            </div>
...
if (!empty($_GET['brand'])) {
    foreach ($_GET['brand'] as $brand) {
        $brandFilter[] = "'" . mysqli_real_escape_string($connection, $brand) . "'";
    }
}

if (!empty($_GET['colour'])) {
    foreach ($_GET['colour'] as $colour) {
        $colourFilter[] = "'" . mysqli_real_escape_string($connection, $colour) . "'";
    }
}
...
if (!empty($brandFilter)) {
    $sqlFilterParts[] = "brand IN (" . implode(',', $brandFilter) . ")";
}
